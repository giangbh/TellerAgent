# Hệ Sinh Thái 24 Công Cụ Model Context Protocol (MCP Ecosystem)

Dưới đây là danh mục chi tiết 24 công cụ ngân hàng chuẩn hóa được triển khai trên nền tảng **B.Smart Teller Copilot**:

---

## 1. Phân Nhóm Năng Lực & 24 Công Cụ
...
### 1.2. Nhóm Phân Tích Chân Dung 360, Tín Dụng, NBO & Lịch Sử Tương Tác Đa Kênh (4 Tools)
...
5. `customer.interaction.history` (`customer_interaction_history`): Trích lục lịch sử tương tác 360 độ: cuộc gọi Contact Center (1900), phản ánh/khiếu nại VOC, đánh giá CSAT/NPS, và lịch sử sự cố lỗi giao dịch (Timeout Napas 24/7, Declined POS, nuốt thẻ/khóa PIN ATM, OTP chậm).
6. `customer.credit.score.check` (`customer_credit_score_check`): Chấm điểm tín dụng CIC (680-820), xếp hạng tín nhiệm AAA/AA/A, kiểm tra nhóm nợ CIC 1 và phê duyệt hạn mức vay/thấu chi trước.
7. `recommendation.nbo.products` (`recommendation_nbo_products`): Đề xuất danh mục sản phẩm tiếp theo Next-Best-Offer (NBO) cá nhân hóa cho từng khách hàng.
